...
numpy.set_printoptions(suppress=True,linewidth=sys.maxsize,threshold=sys.maxsize)
...
class FFTPower(FFTBase):
    ...
    def __init__(self, first, mode, Nmesh=None, BoxSize=None, second=None,
                    los=[0, 0, 1], Nmu=5, dk=None, kmin=0., kmax=None, poles=[], regress=False, test=False):
    ...
    def run(self):
        ...
        result, pole_result = project_to_basis(y3d, edges,
                                               ...
                                               los=self.attrs['los'], regress=self.regress, test=self.test)
...
def project_to_basis(y3d, edges, los=[0, 0, 1], poles=[], regress=True, test=False):
    """
    ...
    *   the `mu` bins are half-inclusive half-exclusive, except the last bin
    ...
    logger   = logging.getLogger('project_to_basis')
        
    # setup the bin edges and number of bins; here x is k.
    ...
    anyodd   = numpy.any(numpy.array(poles) % 2)

    # Is the (cross) power Hermitian Symmetric?  Note, not the underlying r2c. 
    # If providing odd multipoles, assume you want -mu and don't set symmetric.
    hermitian_symmetric = y3d.compressed
    
    if comm.rank == 0:
        print('Hermitian symmetric:  {}'.format(hermitian_symmetric))
    ...
    # 2D (k,mu) binned grid for each multipole weighting. 
    ysum  = numpy.zeros((Nell, Nx+2, Nmu+2), dtype=y3d.dtype) # extra dimension for multipoles

    if regress:
      assert  len(poles) > 0 
        
      # 2D (k,mu) binned grid for each multipole, multipole pair weighting, e.g. L2 x L4.
      lsum = numpy.zeros((Nell, Nell, Nx+2, Nmu+2), dtype=y3d.dtype) 
    
    # Number of modes available in each k, mu grid cell.
    ...
    # iterate over y-z planes of the coordinate mesh, i.e. slabs of constant kx. 
    # y3d.x is the wavenumber array. 
    for slab in SlabIterator(y3d.x, axis=0, symmetry_axis=symmetry_axis):
        ...
        # get the bin indices in k for these points in this slab, (ky, kz), cnst. kz.
        ...
        # lowest value of abs(mu) = 0.0, should be inclusive on zeroth bin. 
        # assert  (not numpy.any(dig_mu) == 0)
        
        # make the multi-index
        multi_index = numpy.ravel_multi_index([dig_x, dig_mu], (Nx+2,Nmu+2))

        # sum up k in each bin (accounting for negative freqs with Hermitian double counting).
        xslab[:]   *= slab.hermitian_weights

        # Here we're adding up k values in the binned k, mu grid, but flatten. 
        ...
        for iell, ell in enumerate(_poles):            
            ...
            if test:
                ...
                symfactor      = numpy.zeros_like(mu) 
                
                for i in range(len(_poles)):
                    factor    += legpoly[i](mu)
                    symfactor += (-1)**_poles[i] * legpoly[i](mu)
                    
                # Assumes isotropic power spectrum passed for test.                                                                                                                                                          
                weighted_y3d   =  legpoly[iell](mu) *    factor * y3d[slab.index]
                weighted_y3d  += legpoly[iell](-mu) * symfactor * y3d[slab.index]
        ...
        if regress:
            for jj, jell in enumerate(_poles):
                for kk, kell in enumerate(_poles): 
                    prod                  = (1 + (-1)**jell * (-1)**kell) * legendre(jell)(mu) * legendre(kell)(mu)
                    
                    '''
                    print(mu.shape, prod.shape, slab.shape, slab.meshshape, numpy.min(mu), numpy.max(mu),\
                          numpy.min(slab.coords(0)), numpy.min(slab.coords(1)), numpy.min(slab.coords(2)),\
                          numpy.max(slab.coords(0)), numpy.max(slab.coords(1)), numpy.max(slab.coords(2)))
                    '''
                    '''
                    print('{:.6e}\t{:.6e}\t{:d}\t{:.6e}\t{:.6e}\t{:d}\t{:.6e}\t{:.6e}\t'.format(numpy.min(mu),\
                                                                                                numpy.max(mu),\
                                                                                                jell,\
                                                                                                numpy.min(legpoly[jj](mu)),\
                                                                                                numpy.max(legpoly[jj](mu)),\
                                                                                                kell,\
                                                                                                numpy.min(legpoly[kk](mu)),\
                                                                                                numpy.max(legpoly[kk](mu))))
            ...
            assert  numpy.all(numpy.isfinite(lsum))
    ...
    with numpy.errstate(invalid='ignore', divide='ignore'):
        ...
        if regress:
            # Summing over last mu axis.
            ysum     = ysum[:,sl,sl].sum(axis=-1)
            lsum     = lsum[..., sl, sl].sum(axis=-1)
                        
            # Deal with one k bin at a time.
            for i in numpy.arange(Nx):
                inv        = pinvh(lsum[..., i]) 
                vec        = ysum[:,i]
                
                poles[:,i] = numpy.dot(inv, vec)

                print('\n\nRegression matrix for k = {}.'.format(xmean_1d[i]))
                
                print(lsum[..., i].real / lsum[0, 0, i].real)
